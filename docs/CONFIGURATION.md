...
## Interval diagnostic limits

No new saved setting is required. Process surveys use `min(max_process_scan, 10000)` for admission; ETS surveys use `min(diagnostics.ets_max_tables, 256)` and at most 5000 ms per metadata pass while respecting a lower `ets_timeout_ms`. Existing ETS concurrency applies. The protocol permits 1000/5000 ms intervals; the UI selects 5000 ms. Stack observations are spaced by 250 ms plus RPC time, at most 20 observations with 20 frames each. The helper job deadline is the requested interval plus 12000 ms. Results contain at most 60 ranked entity rows; the UI shows five per page. Native enumeration may still allocate before a racing population reaches helper-side checks.

Measurement requests, filters, pages and copied reports are session context. There is no continuous profiler, extra closed-panel scan, trace installation or implicit export. The current ZIP still contains its documented snapshot/recorder evidence; a survey report is copied explicitly when needed.
