...
## Interval diagnostic commands (protocol 1)

`process_window`, `ets_window` and `sample_process` use normal asynchronous `job` replies. All require `request_id`, an admitted `node`, nonnegative integer `expected_creation` and `duration_ms` equal to 1000 or 5000. `sample_process` also requires a canonical `pid`. The native UI requests 5000 ms. Authorization checks the live panel/epoch at execution; collectors also check requested and endpoint incarnations.

`process_window` / `ets_window` return `kind,node,creation,from_at_ms,at_ms,span_ms,first_scanned,last_scanned,first_total,last_total,partial,matched,first_only,last_only,omitted_rows,rows,warning`. Each row has an exact `key`, allowlisted identity/current metadata, `status` (`matched`, `newly_observed`, `not_reobserved`), signed `memory_delta_bytes`, `mailbox_delta` or `size_delta`, and process `reductions_delta` / `reductions_per_second` with a `counter_reset` flag. Unavailable or unmatched deltas are null. At most 60 rows are returned, chosen across the relevant rankings. ETS metadata adds `identity`, a digest of the underlying table identifier, distinct from its reusable name; an ownership change has its own flag.

`sample_process` returns exact node/PID/creation, capture endpoints, actual `span_ms`, requested/observed/missing sample counts, `partial`, `outcome`, status counts, resource deltas and `stacks`. A stack has `key,count,frames`; frames contain module/function/arity and optional line, never arguments or paths. At most 20 samples and 20 frames per stack are retained. Frequency uses all observed samples as denominator, including observations without usable frames. Acquisition spacing is 250 ms plus RPC time; observed span is not a promised exact duration.

`cancel_job` takes the existing `request_id`. It cancels only that helper owner's interactive active/queued request. Canceled jobs emit a terminal `job` with `status:canceled`; missing/already-terminal IDs do not generate a new success claim. It cannot cancel independent control recovery or exports. Panel close, historical mode and owner loss retain their existing broader cancellation semantics. No new background task or protocol listener is introduced.
