...
assert.match(panelSource, /padding:\s*Style\.space\(8\)/);
assert.match(panelSource, /spacing:\s*Style\.spacing\.xxl/);
assert.match(panelSource, /id:\s*headerBar[\s\S]*?spacing:\s*Style\.spacing\.huge/);
assert.match(panelSource, /id:\s*identityBlock[\s\S]*?spacing:\s*Style\.spacing\.xxs/);
assert.match(panelSource, /PanelSeparator\s*\{[\s\S]*?id:\s*headerSeparator/);
assert.match(panelSource, /function\s+headerTime\(at\)[\s\S]*?Qt\.formatTime\(new Date\(at\), "HH:mm:ss"\)/);
assert.match(panelSource, /text:\s*root\.stale\s*\?\s*"STALE"\s*:\s*"LIVE"/);
assert.match(panelSource, /id:\s*identityBlock[\s\S]*?text:\s*"BEAM DECK"[\s\S]*?id:\s*headerActions/);
...
assert.match(read("daemon/mix.exs"), /version: "1\.2\.0"/);
assert.match(read("daemon/lib/beam_deck/diagnostics/bundle.ex"), /version: "1\.2\.0"/);
