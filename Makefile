...
check: static
	@if command -v mix >/dev/null 2>&1; then \
		cd daemon && $(MIX_ENV_VARS) mix deps.get && $(MIX_ENV_VARS) mix format --check-formatted && $(MIX_ENV_VARS) MIX_ENV=test mix compile --warnings-as-errors && $(MIX_ENV_VARS) mix test && $(MIX_ENV_VARS) BEAM_DECK_INTEGRATION=1 mix test && $(MIX_ENV_VARS) mix credo --strict && $(MIX_ENV_VARS) mix dialyzer; \
	else \
