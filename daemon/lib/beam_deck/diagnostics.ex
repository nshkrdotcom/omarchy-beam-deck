defmodule BeamDeck.Diagnostics do
  ...
  def handle_call({:submit, owner, id, kind, node, fun, opts}, _from, state) do
    ids =
      Enum.map(state.queue, & &1.id) ++
        Enum.map(Map.values(state.active), & &1.id) ++ state.recent

    cond do
      not BeamDeck.Protocol.request_id?(id) ->
        {:reply, {:error, :invalid_request_id}, state}

      id in ids ->
        {:reply, {:error, :duplicate_request}, state}

      length(state.queue) >= state.config["max_queued_jobs"] ->
  ...
  @impl true
  def handle_cast({:cancel, owner}, state),
    do: {:noreply, cancel(state, owner, true) |> dispatch()}
  ...
  def handle_info(
    ...
    case Map.pop(state.active, ref) do
      ...
      {job, active} ->
        ...
        {:noreply,
         finish(%{state | active: active}, job)
         |> dispatch()}
    end
  end
  ...
  defp handle_owner_down(state, ref) do
    case Enum.find(
           state.owners,
           fn {_owner, monitor} -> monitor == ref end
         ) do
      nil ->
        {:noreply, state}

      {owner, _} ->
        {:noreply, cancel(state, owner, false) |> dispatch()}
    end
  end
  ...
  defp dispatch(state) do
    ...
    if map_size(state.active) < state.config["max_concurrent_jobs"] and not is_nil(index) do
      ...
      task =
        Task.Supervisor.async_nolink(BeamDeck.DiagnosticTasks, fn ->
          try do
            job.fun.()
          rescue
      ...
      send(
        job.owner,
        {:diagnostic,
         %{type: "job", request_id: job.id, kind: job.kind, node: job.node, status: "started"}}
      )
  ...
  defp finish(state, job) do
    ...
    owners =
      if Enum.any?(pending, &(&1.owner == job.owner)) do
        state.owners
      else
  ...
  defp deliver(job, result) do
    ...
    message =
      case result do
        {:ok, data} ->
          Map.merge(base, %{status: "complete", result: data})

        {:error, code} when is_atom(code) ->
          Map.merge(base, %{status: "error", error: Atom.to_string(code)})

        :canceled ->
          Map.put(base, :status, "canceled")

        _ ->
          Map.merge(base, %{status: "error", error: "diagnostic_failed"})
      end
