defmodule BeamDeck.Redaction do
  ...
  def configure(config) do
    ...
    :persistent_term.put({__MODULE__, :secrets}, Enum.filter([cookie | secrets], &(is_binary(&1) and &1 != "")))
    :ok
  end
  ...
  def text(value, cap) when is_binary(value) do
    value |> String.to_charlist() |> Enum.map(fn c -> if c < 32 and c not in [9, 10], do: 32, else: c end)
    |> List.to_string() |> truncate(cap)
  rescue
    _ -> value |> :binary.bin_to_list() |> Enum.map(fn c -> if c in 32..126, do: c, else: 63 end)
      |> List.to_string() |> truncate(cap)
  end
  ...
  defp valid_prefix(part) do
    if String.valid?(part), do: part, else: valid_prefix(binary_part(part, 0, byte_size(part) - 1))
  end

  defp walk(_value, depth, _secrets, _export) when depth > 12, do: "[TRUNCATED]"
  defp walk(value, depth, secrets, export?) when is_map(value) do
    value |> Enum.take(128) |> Enum.reduce(%{}, fn {key, val}, acc ->
      name = key |> text(128) |> String.downcase()
      if name in @forbidden or sensitive_key?(key) or (export? and name in @export_only) do
        acc
      else
        raw_key = if is_binary(key), do: key, else: text(key, 128)
        safe_text = raw_key |> scrub(secrets) |> text(128)
        safe_key = if is_atom(key) and safe_text == raw_key, do: key, else: safe_text
        Map.put(acc, safe_key, walk(val, depth + 1, secrets, export?))
      end
    end)
  end
  ...
  defp walk({key, value}, depth, secrets, export?) when is_atom(key) or is_binary(key) do
    if sensitive_key?(key), do: [text(key, 128), "[REDACTED]"],
  ...
  defp walk(value, _depth, secrets, _export) when is_binary(value), do: value |> scrub(secrets) |> text()
  defp walk(value, _depth, _secrets, _export) when is_number(value) or is_boolean(value) or is_nil(value), do: value
  defp walk(value, _depth, secrets, _export) when is_atom(value), do: value |> Atom.to_string() |> scrub(secrets) |> text()
  defp walk(value, _depth, _secrets, _export) when is_pid(value), do: BeamDeck.Remote.pid_text(value)
  defp walk(_value, _depth, _secrets, _export), do: "[metadata]"

  defp sensitive_key?(key) do
    normalized = key |> text(128) |> String.downcase() |> String.replace(~r/[^a-z0-9]/, "")
    Regex.match?(~r/(password|passwd|secret|token|cookie|credential|credentials|authorization|apikey|privatekey)$/, normalized)
  end
  ...
  defp scrub(value, secrets) do
    ...
    value = Enum.reduce(Enum.sort_by(secrets, &byte_size/1, :desc), value, fn secret, acc -> String.replace(acc, secret, "[REDACTED]") end)
    Regex.replace(~r/((?:-setcookie|--cookie|cookie|password|token|secret)\s*[=: ]\s*)[^\s,;"}]+/i,
      value, "\\1[REDACTED]")
  end
end
