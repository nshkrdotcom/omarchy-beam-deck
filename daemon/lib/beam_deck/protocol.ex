defmodule BeamDeck.Protocol do
  ...
  defp validate(cmd, map) do
    checks = [
      ...
      cmd not in ~w(inspect_process inspect_ets set_schedulers set_dirty_schedulers gc restore deep_events) or
        node_name?(map["node"]),
      ...
      cmd != "compare_frames" or
        (request_id?(map["from_frame_id"]) and request_id?(map["to_frame_id"])),
      ...
      cmd not in ~w(set_schedulers set_dirty_schedulers) or
        (is_integer(map["value"]) and map["value"] in 1..1024),
      cmd != "gc" or not Map.has_key?(map, "expected_creation") or
