defmodule BeamDeck.Diagnostics.Process do
  ...
  alias BeamDeck.{Remote, Redaction}
  @fields [:registered_name, :status, :current_function, :initial_call, :current_stacktrace,
    :message_queue_len, :memory, :reductions, :heap_size, :total_heap_size, :stack_size]

  def inspect_process(node, text, opts) do
    deadline = System.monotonic_time(:millisecond) + opts["process_timeout_ms"]
    with {:ok, creation} when is_integer(creation) <- rpc(node, :erlang, :system_info, [:creation], deadline),
         {:ok, pid} <- Remote.parse_pid(node, text),
         {:ok, info} when is_list(info) <- rpc(node, :erlang, :process_info, [pid, @fields], deadline) do
      data = Map.new(info)
      {ancestry, warnings} = ancestry(node, pid, opts, deadline)
      binaries = binaries(node, pid, opts, deadline)
      result = %{
        node: Atom.to_string(node), creation: creation, pid: text, at_ms: System.system_time(:millisecond),
        registered_name: if(is_atom(data[:registered_name]) and data[:registered_name] not in [nil, :undefined], do: Atom.to_string(data[:registered_name]), else: nil),
        status: data[:status], current_function: frame(data[:current_function]),
        initial_call: frame(data[:initial_call]),
        stack: Enum.map(Enum.take(data[:current_stacktrace] || [], opts["process_stack_frames"]), &frame/1),
        mailbox: data[:message_queue_len], memory_bytes: data[:memory], reductions: data[:reductions],
        heap_words: data[:heap_size], total_heap_words: data[:total_heap_size], stack_words: data[:stack_size],
        ancestry: ancestry, binaries: binaries, warnings: warnings ++ binary_warning(binaries),
        exclusions: "No state, message contents, dictionary dump, binary contents or arbitrary evaluation."
      }
  ...
  def frame({mod, fun, args, location}) when is_atom(mod) and is_atom(fun) do
    arity = if is_integer(args), do: args, else: if(is_list(args), do: length(Enum.take(args, 256)), else: nil)
  ...
  def binary_summary(rows, cap) when is_list(rows) do
    ...
    valid = for {id, size, refs} <- selected, is_integer(id), is_integer(size), is_integer(refs),
      do: %{id: id, bytes: size, references: refs}
    unique = Enum.uniq_by(valid, & &1.id)
    malformed = length(valid) != length(selected)
    %{status: if(malformed, do: "binary_summary_unavailable", else: "available"), sampled_entries: length(selected), total_entries: length(rows),
      partial: malformed or length(rows) > cap, referenced_bytes: if(malformed, do: nil, else: Enum.sum(Enum.map(unique, & &1.bytes))),
      top: if(malformed, do: [], else: unique |> Enum.sort_by(& &1.bytes, :desc) |> Enum.take(10) |> Enum.map(&Map.drop(&1, [:id]))),
      accounting: "Observed off-heap references; shared bytes are not exclusive process ownership."}
  end
  ...
  defp binary_warning(%{partial: true}), do: ["Binary reference metadata is incomplete or unavailable."]
  ...
  defp ancestry(node, pid, opts, deadline) do
    ...
    case rpc(node, :erlang, :process_info, [pid, {:dictionary, :"$ancestors"}], deadline) do
      {:ok, {{:dictionary, :"$ancestors"}, ancestors}} when is_list(ancestors) ->
        ...
        {rows, _child} = Enum.map_reduce(selected, pid, fn ancestor, child ->
          case ancestor_pid(node, ancestor, deadline) do
            ...
            _ -> {%{name: ancestor_name(ancestor), status: "unavailable"}, child}
          end
        end)
        warnings = if length(ancestors) > length(selected), do: ["Ancestry depth capped."], else: []
        {rows, warnings}
      {:ok, {{:dictionary, :"$ancestors"}, :undefined}} -> {[], []}
      _ -> {[], ["Focused ancestry is unavailable; no dictionary fallback was attempted."]}
    end
  end
  ...
  defp small_supervisor_child(node, parent, child, deadline) do
    with {:ok, counts} when is_list(counts) <- rpc(node, :supervisor, :count_children, [parent], deadline),
         ...
         {:ok, rows} when is_list(rows) <- rpc(node, :supervisor, :which_children, [parent], deadline),
         row when not is_nil(row) <- Enum.find(Enum.take(rows, 100), fn {_id, pid, _type, _modules} -> pid == child end) do
      child_row(row)
    else
  ...
  defp child_row({id, pid, type, modules}) do
    ...
    %{id: Redaction.text(id, 120), pid: Remote.pid_text(pid), type: type,
      modules: if(is_list(modules), do: Enum.take(modules, 8) |> Enum.map(&Redaction.text(&1, 120)), else: [])}
  end
  defp rpc(node, mod, fun, args, deadline) do
    remaining = deadline - System.monotonic_time(:millisecond)
    if remaining > 0, do: Remote.call_raw(node, mod, fun, args, remaining), else: {:error, :timeout}
  end
end
