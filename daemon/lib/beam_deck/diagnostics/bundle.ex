defmodule BeamDeck.Diagnostics.Bundle do
  ...
  @limit 16_777_216
  def directory do
    Path.join([System.get_env("XDG_STATE_HOME") || Path.join(System.user_home!(), ".local/state"), "beam-deck", "exports"])
  end
  def export(snapshot, recorder, from, to, dir \\ directory()) do
    with {:ok, frames} <- FlightRecorder.range(recorder, from, to),
         ...
         path <- Path.join(dir, "beam-deck-" <> Integer.to_string(System.system_time(:millisecond)) <> "-" <> Integer.to_string(System.unique_integer([:positive])) <> ".zip"),
         :ok <- PrivateFile.write(path, archive) do
      {:ok, %{path: path, bytes: byte_size(archive), frame_count: length(frames),
        privacy: "Redacted operational metadata may still identify applications. Review before sharing."}}
    else
  ...
  defp encode_frames(frames) do
    result = Enum.reduce_while(frames, {[], 2}, fn frame, {encoded, size} ->
  ...
  defp entries(snapshot, frames_json, frames) do
    fields = Map.take(snapshot, ~w(type protocol at_ms session_id host summary nodes alerts forecasts budget budget_trial topology)a)
    metadata = %{product: "BEAM Deck", version: "1.1.0", protocol: 1, agentless: true,
      exported_at_ms: System.system_time(:millisecond), frame_count: length(frames),
      from_at_ms: if(frames == [], do: nil, else: hd(frames).at_ms),
      to_at_ms: if(frames == [], do: nil, else: List.last(frames).at_ms)}
    data = [
      {~c"BEAM-DECK-DIAGNOSTICS.txt", "BEAM Deck 1.1 diagnostics\nReview before sharing. Node, module and registered names are operational metadata.\nNo process state/messages, ETS contents, cookies, raw config/logs or crash dump are included.\n"},
    ...
    if Enum.sum(Enum.map(data, fn {_, bytes} -> byte_size(bytes) end)) <= @limit, do: {:ok, data}, else: {:error, :export_too_large}
  end
end
