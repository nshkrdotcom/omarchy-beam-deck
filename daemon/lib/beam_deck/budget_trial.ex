defmodule BeamDeck.BudgetTrial do
  ...
  def begin(rows, budget, nodes, epoch, lease), do: GenServer.call(__MODULE__, {:begin, rows, budget, nodes, epoch, lease}, 30_000)
  ...
  def validate(rows, budget, nodes) do
    normalized = Enum.map(rows, fn row -> %{node: row["node"], current: row["current"], suggested: row["suggested"]} end)
    eligible = Enum.all?(normalized, fn row ->
      node = Enum.find(nodes, &(&1.name == row.node))
      not is_nil(node) and node[:attached] == true and node[:local] == true and node[:schedulers_online] == row.current and
        is_integer(row.suggested) and row.suggested >= 1 and row.suggested <= row.current
    end)
    if eligible and length(rows) in 1..16 and length(Enum.uniq_by(normalized, & &1.node)) == length(rows) and
  ...
  @impl true
  def handle_cast({:request, owner, id, {action, _trial_id} = command}, state) when action in [:keep, :revert] do
  ...
  def handle_call({:begin, rows, budget, nodes, epoch, lease}, from, state) do
    with true <- state.panel and state.epoch == epoch and not busy?(state),
      ...
      targets = Map.new(nodes, fn node ->
        {node.name, %{creation: node[:creation], os_pid: to_string(node[:os_pid] || "")}}
      end)
      trial = %{trial_id: id, targets: targets, status: "applying", rows: accepted, pending: accepted, journal: [],
        failures: [], reason: nil, from: from, timer: timer, remaining: [],
        started_at_ms: System.system_time(:millisecond),
        before_metrics: Enum.map(nodes, &Map.take(&1, [:name, :run_queue, :processes, :memory, :schedulers_online])),
  ...
  def handle_call({:set, name, flag, value}, _from, state) do
    ...
    if busy?(state) do
      ...
      with {:ok, node} <- Discovery.existing_node(name), {:ok, identity} <- Remote.identity(node),
           {:ok, current} when is_integer(current) <- Remote.call_raw(node, :erlang, :system_info, [flag]) do
        ...
        case Remote.set_flag(node, flag, value) do
          {:ok, old} ->
            ...
            {:reply, {:ok, %{previous: old, applied: value}}, %{next | originals: remember(state.originals, corrected)}}
          _ -> {:reply, {:error, :mutation_outcome_uncertain_use_restore}, next}
        end
      else
  ...
  def handle_call({:restore, name}, _from, state) do
    if busy?(state) do
      ...
      result = if failures == [], do: {:ok, %{restored: true}}, else: {:ok, %{restored: false, failures: failures}}
      ...
      [entry | rest] ->
        case restore_entry(entry) do
          :ok ->
            ...
            {:noreply, next}
          {:error, reason} ->
            failure = %{node: entry.node, error: reason, original: entry.original, applied: entry.applied}
            next = %{state | trial: %{trial | remaining: rest, failures: [failure | trial.failures]}}
            send(self(), {:step, id})
            {:noreply, next}
        end
    end
  end
  def handle_info({:expire, id}, %{trial: %{trial_id: id}} = state), do: {:noreply, start_rollback(state, :lease_expired)}
  ...
  defp apply_row(state, row) do
    Process.put(:beam_deck_rpc_deadline, System.monotonic_time(:millisecond) + 1_500)
    with {:ok, node} <- Discovery.existing_node(row.node), {:ok, identity} <- Remote.identity(node),
      ...
      entry = %{node: row.node, flag: :schedulers_online, original: current, applied: row.suggested, identity: identity}
      # Journal BEFORE requesting a side effect: an RPC timeout is not proof of no mutation.
      next = %{state | trial: %{state.trial | journal: [entry | state.trial.journal]}, originals: remember(state.originals, entry)}
      case Remote.set_flag(node, :schedulers_online, row.suggested) do
        {:ok, ^current} -> send(self(), {:step, state.trial.trial_id}); notify(next); next
        {:ok, actual_old} ->
          corrected = %{entry | original: actual_old}
          journal = [corrected | tl(next.trial.journal)]
          start_rollback(%{next | originals: remember(state.originals, corrected), trial: %{next.trial | journal: journal}}, :concurrent_change)
        _ -> start_rollback(next, :apply_failed)
      end
    else
  ...
  defp start_rollback(%{trial: %{status: status}} = state, _reason) when status in ["kept", "reverted", "reverting"], do: state
  defp start_rollback(state, reason) do
    t = state.trial
    Process.cancel_timer(t.timer)
    next = %{state | trial: %{t | status: "reverting", reason: reason, remaining: t.journal, failures: []}}
    send(self(), {:step, t.trial_id})
    notify(next)
    next
  end
  defp busy?(%{trial: nil}), do: false
  defp busy?(state), do: state.trial.status in ["applying", "active", "reverting", "rollback_failed"]

  defp remember(originals, entry) do
    key = {entry.node, entry.flag}
    case originals[key] do
      %{identity: identity} = old when identity == entry.identity -> Map.put(originals, key, %{entry | original: old.original})
      _ -> Map.put(originals, key, entry)
    end
  end
  defp after_rollback(originals, entry) do
    ...
    case originals[key] do
      nil -> originals
      old ->
        if old.original == entry.original, do: Map.delete(originals, key), else: Map.put(originals, key, %{old | applied: entry.original})
    end
  end
  defp restore_entries(originals, name) do
    Enum.reduce(originals, {originals, []}, fn {key, entry}, {acc, failures} ->
      if name == :all or entry.node == name do
        case restore_entry(entry) do
          :ok -> {Map.delete(acc, key), failures}
          {:error, error} -> {acc, [%{node: entry.node, flag: entry.flag, error: error} | failures]}
        end
      else
        {acc, failures}
      end
  ...
  defp restore_entry(entry) do
    Process.put(:beam_deck_rpc_deadline, System.monotonic_time(:millisecond) + 1_500)
    with {:ok, node} <- Discovery.existing_node(entry.node), {:ok, identity} <- Remote.identity(node),
         true <- identity == entry.identity,
         {:ok, current} <- Remote.call_raw(node, :erlang, :system_info, [entry.flag]) do
      cond do
        current == entry.original -> :ok
        current != entry.applied -> {:error, :externally_changed}
        true ->
          case Remote.set_flag(node, entry.flag, entry.original) do
            {:ok, _} ->
              if Remote.call(node, :erlang, :system_info, [entry.flag], nil) == entry.original,
                do: :ok, else: {:error, :restore_unconfirmed}
  ...
  defp public(trial) do
    Map.take(trial, [:trial_id, :status, :rows, :failures, :reason, :started_at_ms, :expires_at_ms, :before_metrics])
