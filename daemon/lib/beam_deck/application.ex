defmodule BeamDeck.Application do
  ...
  def start(_type, _args) do
    ...
    config = case BeamDeck.Config.load() do
      {:ok, config, _} -> config
      {:error, _, defaults, _} -> defaults
    end
    children = [
      Supervisor.child_spec({Task.Supervisor, name: BeamDeck.DiagnosticTasks}, id: :diagnostic_tasks),
      Supervisor.child_spec({Task.Supervisor, name: BeamDeck.CollectionTasks}, id: :collection_tasks),
