defmodule BeamDeck.CrashDump do
  ...
  def fingerprint(cwd) when is_binary(cwd) and cwd != "" do
    case File.lstat(Path.join(cwd, "erl_crash.dump"), time: :posix) do
      {:ok, %{type: :regular} = stat} ->
        Map.take(stat, [:inode, :size, :mtime, :ctime, :major_device])
  ...
  def disappeared(previous, current, at) do
    ...
    previous.runtimes
    |> Enum.reject(&MapSet.member?(identities, {&1.pid, &1[:starttime]}))
    |> Enum.take(16)
    |> Enum.map(fn runtime ->
      %{
        id: "#{runtime.pid}-#{runtime[:starttime] || 0}-#{at}",
        pid: runtime.pid,
        at_ms: at,
        node: runtime[:node_name],
        cwd: runtime[:cwd],
        previous_fingerprint: runtime[:crash_dump_fingerprint],
        status: "checking"
      }
    end)
  end
  ...
  def triage(exit, opts) do
    ...
    result =
      if first.status in ["missing", "unchanged", "unmatched"] do
  ...
  defp read_match(exit, opts) do
    ...
    cond do
      ...
      fp == exit.previous_fingerprint ->
        %{status: "unchanged"}

      abs(fp.mtime * 1_000 - exit.at_ms) > opts["crash_dump_match_window_ms"] ->
        %{status: "unmatched"}

      true ->
        bounded_header(Path.join(exit.cwd, "erl_crash.dump"), fp, opts["crash_dump_read_bytes"])
    end
  end
      ...
      {:ok, io} ->
        try do
          with {:ok, record} <- :file.read_file_info(io, time: :posix),
               ...
               true <-
                 stat.type == :regular and stat.inode == expected.inode and
            ...
            |> Map.merge(%{
              ...
              prefix_only: stat.size > byte_size(bytes),
