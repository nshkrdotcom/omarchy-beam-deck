defmodule BeamDeck.IncidentsTest do
  ...
  defp snapshot(at, alerts), do: %{at_ms: at, alerts: alerts, nodes: [], forecasts: [], events: [], crash_triage: []}
  test "persistent conditions retain identity and resolve after two quiet polls" do
    a = %{id: "n.process_limit", node: "n", title: "Process table pressure", severity: "warning", message: "90%"}
  ...
  test "mailbox evidence matches PID and time window, including a hot process beyond first three" do
    alert = %{id: "api@host.<0.7.0>.mailbox", node: "api@host", process: "<0.7.0>", title: "Mailbox", severity: "warning", message: "Backlog"}
    node = %{name: "api@host", hot_processes_at_ms: 19500, hot_processes: for(i <- 1..7, do: %{pid: "<0.#{i}.0>", reductions: i, name: "worker"})}
    events = [%{id: "yes", node: "api@host", subject: "<0.7.0>", at_ms: 13000, kind: "long_gc"},
      %{id: "old", node: "api@host", subject: "<0.7.0>", at_ms: 1000, kind: "long_schedule"},
      %{id: "other", node: "api@host", subject: "<0.8.0>", at_ms: 13000, kind: "long_message_queue"}]
    s = snapshot(20000, [alert]) |> Map.merge(%{nodes: [node], events: events})
    ...
    assert Enum.any?(i.evidence, &(&1[:processes] == [%{pid: "<0.7.0>", reductions: 7, name: "worker"}]))
  end

  test "forecast and threshold share an incident without upgrading heuristics to observations" do
    a = %{id: "api@host.atom_limit", node: "api@host", title: "Atoms", severity: "warning", message: "80%"}
    f = %{node: "api@host", metric: "atoms", kind: "capacity", summary: "Trend", severity: "critical", eta_ms: 1000,
      confidence: 0.9, rate_per_second: 10, current: 80, limit: 100, span_ms: 60000, sample_count: 20}
  ...
  test "resolved incidents expire and watchlist missing is not a crash claim" do
    s = Map.put(snapshot(1, []), :watchlist, %{entries: [%{"id" => "p", "status" => "missing", "label" => "Orders", "node" => "api@host"}]})
    [i] = Incidents.update([], s, Config.defaults())
