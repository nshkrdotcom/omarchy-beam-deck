defmodule BeamDeck.RedactionTest do
  ...
  test "recursively removes prohibited categories and exact secrets" do
    data = %{text: "cookie=abc secret-value", messages: ["never"],
  ...
  test "export removes host paths, commands and raw configuration" do
    out = Redaction.export(%{cwd: "/private", argv: ["secret"], config_path: "/home/u", status: "ok"})
    assert out == %{status: "ok"}
  end

  test "sensitive concepts and key/value lists are scrubbed without redacting harmless counts" do
    data = %{api_key: "never", privateKey: "never", passwd: "never", access_token: "never",
      key_count: 4, entries: [authorization: "never", credentials: "never", size: 3]}
  ...
  test "an exact configured secret cannot escape through atom keys or values" do
    sanitized = BeamDeck.Redaction.sanitize(%{test_secret_atom: :test_secret_atom}, secrets: ["test_secret_atom"])
