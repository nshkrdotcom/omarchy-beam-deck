defmodule BeamDeck.CrashDumpTest do
  ...
  test "real file matching reads only a bounded new regular-file prefix" do
    ...
    bytes = "=erl_crash_dump:0.5\nTimestamp\nSlogan: controlled test\n=scheduler:1\nCurrent Process: <0.9.0>\nCurrent Function: app:run/0\n=memory\nbinary: 1024\n=proc:<0.9.0>\n" <> String.duplicate("PRIVATE_REMAINDER", 2000)
    File.write!(path, bytes)
    opts = BeamDeck.Config.defaults()["diagnostics"] |> Map.put("crash_dump_read_bytes", 1024) |> Map.put("crash_dump_retry_ms", 1)
    exit = %{id: "test", at_ms: System.system_time(:millisecond), pid: 42, cwd: dir, previous_fingerprint: nil}
    ...
    assert CrashDump.triage(%{exit | previous_fingerprint: CrashDump.fingerprint(dir)}, opts).status == "unchanged"
  ...
  test "a dump created during the bounded retry is found without traversing elsewhere" do
    ...
    File.mkdir_p!(dir)
    on_exit(fn -> File.rm_rf!(dir) end)
    exit = %{id: "retry", at_ms: System.system_time(:millisecond), pid: 42, cwd: dir, previous_fingerprint: nil}
    task = Task.async(fn ->
      Process.sleep(20)
      File.write!(Path.join(dir, "erl_crash.dump"), "=erl_crash_dump:0.5\nTest\nSlogan: delayed\n=end\n")
    end)
