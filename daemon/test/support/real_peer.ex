defmodule BeamDeck.TestPeer do
  ...
  def start do
    ...
    {:ok, peer, node} = :peer.start(%{name: name, wait_boot: 15_000,
      args: [~c"+S", ~c"3:3", ~c"+SDcpu", ~c"1:1", ~c"+SDio", ~c"1",
        ~c"-setcookie", ~c"beam_deck_test_cookie"]})
    Enum.each(["bd_test_worker", "bd_test_sup"], fn name ->
      file = Path.expand("../fixtures/#{name}.erl", __DIR__)
      compiled = :compile.file(String.to_charlist(file), [:binary, :return_errors, :return_warnings])
      {module, binary} = case compiled do
        {:ok, module, binary} -> {module, binary}
        {:ok, module, binary, []} -> {module, binary}
        other -> raise "fixture compilation failed: #{inspect(other)}"
      end
      {:ok, {:module, ^module}} = BeamDeck.Remote.call_raw(node, :code, :load_binary, [module, String.to_charlist(file), binary])
    end)
  ...
  def eventually(fun, attempts) do
    if fun.(), do: :ok, else: (Process.sleep(25); eventually(fun, attempts - 1))
  end
end
