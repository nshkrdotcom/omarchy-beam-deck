defmodule BeamDeck.ForecastTest do
  ...
  defp history(metric, values, step \\ 6_000) do
    ...
    |> Enum.with_index()
    |> Enum.map(fn {value, i} ->
      n = %{name: "dev@host", attached: true, uptime_ms: i * step + 1,
        processes: 100, process_limit: 100_000, atoms: 100, atom_limit: 100_000,
        ports: 10, port_limit: 65_536, memory: %{binary: 0, ets: 0}, ets: 0}
  ...
  test "sample span, duplicate timestamps, sawtooth and distant horizons suppress claims" do
    ...
    assert Forecast.derive(history(:atoms, Enum.map(1..20, &(if rem(&1, 2) == 0, do: 30_000, else: 500))), Config.defaults()) == []
  ...
  test "process and port capacities forecast and classify displayed ETA boundaries" do
    ...
    for {critical, warning, severity} <- [{480_000, 600_000, "critical"}, {1000, 480_000, "warning"}, {1000, 2000, "info"}] do
      config = put_in(Config.defaults(), ["forecast", "critical_eta_ms"], critical)
  ...
  test "growth signals have no capacity or ETA and require absolute and fractional growth" do
    ...
    for metric <- [:binary, :ets] do
      data = history(:atoms, List.duplicate(100, 20)) |> Enum.map(fn frame ->
        i = div(frame.at_ms, 6000)
        n = hd(frame.nodes)
        %{frame | nodes: [Map.put(n, :memory, Map.put(n.memory, metric, 100_000_000 + i * 4_000_000))]}
      end)
      [f] = Forecast.derive(data, Config.defaults())
      assert f.kind == "growth" and is_nil(f.limit) and is_nil(f.eta_ms)
      high_absolute = Config.defaults() |> put_in(["forecast", "binary_growth_min_bytes"], 999_000_000)
        |> put_in(["forecast", "ets_memory_growth_min_bytes"], 999_000_000)
      assert Forecast.derive(data, high_absolute) == []
      high_fraction = Config.defaults() |> put_in(["forecast", "binary_growth_min_fraction"], 0.99)
  ...
  test "incarnation change resets history even when uptime alone would look continuous" do
    rows = history(:atoms, Enum.map(1..20, &(&1 * 1000))) |> Enum.map(fn f ->
