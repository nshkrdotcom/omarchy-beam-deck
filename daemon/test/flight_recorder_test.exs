defmodule BeamDeck.FlightRecorderTest do
  ...
  defp snapshot(at, rss) do
    %{at_ms: at, summary: %{beam_rss_bytes: rss}, host: %{runtimes: []},
      nodes: [], events: [], alerts: [], forecasts: [], history: ["NOT RETAINED"]}
  end
  ...
  test "nearest deep frame is distance and incarnation constrained" do
    n = %{name: "api@host", attached: true, creation: 1, hot_processes_at_ms: 1000, hot_processes: [%{pid: "<0.1.0>"}]}
    s = Map.put(snapshot(1100, 1), :nodes, [n])
    r = FR.push(FR.new(), s, 10)
    assert {:ok, %{frame_id: "frame-1", sampled_at_ms: 1000}} = FR.nearest_deep(r, "api@host", 2000, 1000, 1)
  ...
  test "frame diff does not subtract counters across incarnations" do
    ...
    r = FR.new() |> FR.push(Map.put(snapshot(1000, 1), :nodes, [a]), 10)
  ...
  test "export ranges follow sequence even when the wall clock moves backward" do
    r = FR.new() |> FR.push(snapshot(3000, 1), 10)
      |> FR.push(snapshot(1000, 2), 10) |> FR.push(snapshot(2000, 3), 10)
