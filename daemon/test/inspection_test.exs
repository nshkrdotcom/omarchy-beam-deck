defmodule BeamDeck.InspectionTest do
  ...
  alias BeamDeck.Diagnostics.{Process, Ets}
  test "stack normalization does not expose function arguments" do
    frame = Process.frame({My.Module, :call, [%{secret: "not-output"}], [line: 42, file: ~c"private/path"]})
  ...
  test "binary address identifiers never leave normalization and unknown shape is explicit" do
    assert %{top: [%{bytes: 10, references: 2}]} = Process.binary_summary([{91792384, 10, 2}], 5000)
    assert %{status: "binary_summary_unavailable", partial: true, top: []} = Process.binary_summary([{:unexpected, "private"}], 5000)
  end
