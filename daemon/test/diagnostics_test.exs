defmodule BeamDeck.DiagnosticsTest do
  ...
  test "results are structured and duplicate active IDs are rejected" do
    assert :ok = Diagnostics.submit(self(), "one", "test", "n", fn -> Process.sleep(50); {:ok, %{v: 1}} end)
    assert {:error, :duplicate_request} = Diagnostics.submit(self(), "one", "test", "n", fn -> :ok end)
  ...
  test "deadline cancels the worker and the next job still completes" do
    :ok = Diagnostics.submit(self(), "slow", "test", "n", fn -> Process.sleep(5_000) end, timeout: 20)
  ...
  test "panel cancellation does not cancel an explicit export" do
    ...
    Diagnostics.submit(self(), "export", "export_bundle", nil, fn -> Process.sleep(50); {:ok, %{path: "private"}} end, interactive: false)
  ...
  test "worker exceptions are bounded, and do not expose exception text" do
    ...
    assert_receive {:diagnostic, %{request_id: "raises", status: "error", error: "diagnostic_failed"}}, 1000
    assert Process.alive?(Process.whereis(Diagnostics))
  end

  test "global and per-node concurrency leave a bounded explicit queue" do
    parent = self()
    hold = fn -> send(parent, {:held_worker, self()}); receive do :release -> {:ok, %{}} end end
